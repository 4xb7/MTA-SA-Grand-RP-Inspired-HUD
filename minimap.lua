...
local settings = {
    ...
    aspectRatio = 16/9, -- 3:2 ratio, height is displayScale divided by this
    ...
    map = {
        ...
        backgroundColor = tocolor(87, 111, 126), -- rendered behind map (water color)
    },
    ...
    healthbar = {
        colorOk = {175, 33, 37}, -- {102, 204, 102}
        colorLow = {175, 33, 37}, -- {200, 200, 0}
        colorCritical = {175, 33, 37}, -- {200, 0, 0}
    },
    armorbar = {
        color = {235, 228, 233}, -- {0, 102, 255}
    },
    oxygenbar = {
        alwaysRender = false, -- false = only when oxygen not full or player is in water
        color = {180, 205, 240}, -- {255, 255, 0}
    },
    blips = {
        enabled = true,
        sizeFactor = 12,
...
function render()
    ...
    if (not isPlayerMapVisible()) then
        ...
        radarWidth = mapDisplayWidth + offset + offset
        local radarHeight = mapDisplayHeight + offset + barsHeight + offset + offset
        radarLeft = settings.safeX
        radarTop = sy - settings.safeY - radarHeight
        ...
        local blipSize = 2 * settings.blips.sizeFactor - 6 -- edited by saif
